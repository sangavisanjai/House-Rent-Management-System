//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace HouseRentManagementSystem.Models
{
    using System;
    using System.Collections.Generic;
    
    public partial class House
    {
        public int PropId { get; set; }
        public string PropName { get; set; }
        public string PropType { get; set; }
        public string PropStatus { get; set; }
        public int PropRent { get; set; }
        public string PropAddress { get; set; }
        public long PropContact { get; set; }
        public string PropDescription { get; set; }
        public string PropSize { get; set; }
        public string PropFacing { get; set; }
        public string PetFriendly { get; set; }
        public string AvailableFor { get; set; }
        public string Flooring { get; set; }
        public string AvailableFrom { get; set; }
        public byte[] HImg1 { get; set; }
        public string Email { get; set; }
        public byte[] HImg2 { get; set; }
        public byte[] HImg3 { get; set; }
        public byte[] HImg4 { get; set; }
        public byte[] HImg5 { get; set; }
        public byte[] HImg6 { get; set; }
        public byte[] HImg7 { get; set; }
        public byte[] HImg8 { get; set; }
        public byte[] HImg9 { get; set; }
        public byte[] HImg10 { get; set; }
        public string BookDetails { get; set; }
    }
}
